...
class db_updater():
  ...
  def insert_update_fighter_stats(self, fighter_id, fighter_profile):
    ...
    dic = {
      ...
      'win_ratio': round(fighter_profile['win'] / (fighter_profile['win'] + fighter_profile['loss']) * 100),
  ...
  # create a new database from scratch and write json data ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  def create_new_db_and_write_json_data(self):
    # create tables in a new database ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def create_tables_in_new_db():
      try:
        with closing(sqlite3.connect(self.database)) as db_connection:
          with closing(db_connection.cursor()) as cursor:
        ...
        # print error message
        traceback.print_exc()
        logger.error(f"Unable to create tables for '{self.database}'")
      else:
        logger.info(f"Create tables successful for database'{self.database}'")
    ...
    logger = set_logger()
    create_tables_in_new_db()
    self.update_db()
    
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def main():
  # creating a new db and updating with all data in json ~~~
  #updater = db_updater('ufc_db1.db')
  #updater.create_new_db_and_write_json_data()
